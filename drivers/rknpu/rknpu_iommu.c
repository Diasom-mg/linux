...
int rknpu_iommu_dma_map_sg(struct device *dev, struct sg_table *sgt,
			   enum dma_data_direction dir, bool iova_aligned)
{
	...
	if (iova_aligned)
		return dma_map_sg(dev, sgt->sgl, sgt->nents, dir);
	...
	if (!iova_len) {
		ret = __finalise_sg(dev, sgt->sgl, sgt->nents, 0);
		goto out;
	}
	...
	ret = iommu_map_sgtable(domain, iova, sgt, prot);
	if (ret < 0 || ret < iova_len) {
		LOG_ERROR("failed to map SG: %zd\n", ret);
		goto out_free_iova;
	}

	return __finalise_sg(dev, sgt->sgl, sgt->nents, iova);

out_free_iova:
	rknpu_iommu_dma_free_iova(cookie, iova, iova_len, iova_aligned);
out_restore_sg:
	__invalidate_sg(sgt->sgl, sgt->nents);
out:
